...
export function Alert({ className, variant = "info", icon, title, children, ...props }: AlertProps) {
  ...
  return (
    <div
      role="note"
      className={cn("rounded-xl border-l-4 p-4 flex gap-3 items-start text-sm", variants[variant], className)}
      {...props}
    >
      {icon && <span className="text-lg flex-shrink-0 mt-0.5" aria-hidden="true">{icon}</span>}
      <div>
